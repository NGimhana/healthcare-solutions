...
CREATE USER 'epic'@'%' IDENTIFIED BY '1234';
GRANT ALL PRIVILEGES ON EPIC_HC_ALERT_DB.* TO 'epic'@'%';
USE EPIC_HC_ALERT_DB;
CREATE TABLE SubscribedPatient (
    patientId VARCHAR(255) ,
	shouldMonitor TINYINT(1),
	EMR_PLATFORM VARCHAR(120), 
...
CREATE TABLE MedicationOrder (
    ...
    FOREIGN KEY(patientId) REFERENCES Patient(patientId)
	ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY(prescriberId) REFERENCES Practitioner(prescriberId)
    ON DELETE CASCADE ON UPDATE CASCADE 		
);
